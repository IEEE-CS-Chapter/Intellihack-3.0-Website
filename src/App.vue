...
export default ({
  mounted: () => {
    const heroBg = document.querySelector<HTMLCanvasElement>("#bg")
    // Three JS Template
    //----------------------------------------------------------------- BASIC parameters
    let renderer = new WebGLRenderer({ antialias: true, canvas: heroBg || undefined });
    ...
    let camera = new PerspectiveCamera(20, window.innerWidth / window.innerHeight, 1, 500);

    camera.position.set(0, 2, 14);

    let scene = new Scene();
    let city = new Object3D();
    let smoke = new Object3D();
    let town = new Object3D();
    ...
    scene.fog = new Fog(setcolor, 10, 16);
    //scene.fog = new FogExp2(setcolor, 0.05);
    //----------------------------------------------------------------- RANDOM Function
    ...
    function init() {
      let segments = 2;
      for (let i = 1; i < 40; i++) {
        let geometry = new CubeGeometry(1, 0, 0, segments, segments, segments);
        ...
        let wmaterial = new MeshLambertMaterial({
          color: 0xFFFFFF,
          wireframe: true,
          transparent: true,
          opacity: 0.03,
      ...
      shading:FlatShading*/});
        ...
        let wire = new Mesh(geometry, wmaterial);
        let floor = new Mesh(geometry, material);
        let wfloor = new Mesh(geometry, wmaterial);
      ...
      let gmaterial = new MeshToonMaterial({ color: 0xFFFF00, side: DoubleSide });
      let gparticular = new CircleGeometry(0.01, 3);
      let aparticular = 5;

      for (let h = 1; h < 300; h++) {
        let particular = new Mesh(gparticular, gmaterial);
        particular.position.set(mathRandom(aparticular), mathRandom(aparticular), mathRandom(aparticular));
        particular.rotation.set(mathRandom(), mathRandom(), mathRandom());
        smoke.add(particular);
      };

      let pmaterial = new MeshPhongMaterial({
        color: 0x000000,
        side: DoubleSide,
        opacity: 0.9,
        transparent: true
      });
      let pgeometry = new PlaneGeometry(60, 60);
      let pelement = new Mesh(pgeometry, pmaterial);
    ...
    //----------------------------------------------------------------- Lights
    let ambientLight = new AmbientLight(0xFFFFFF, 4);
    let lightFront = new SpotLight(0xFFFFFF, 20, 10);
    let lightBack = new PointLight(0xFFFFFF, 0.5);

    let spotLightHelper = new SpotLightHelper(lightFront);
    ...
    //----------------------------------------------------------------- GRID Helper
    let gridHelper = new GridHelper(60, 120, 0xFF0000, 0x000000);
    city.add(gridHelper);
    ...
    let createCars = function (cScale = 2, cPos = 20, cColor = 0xFFFF00) {
      let cMat = new MeshToonMaterial({ color: cColor, side: DoubleSide });
      let cGeo = new BoxGeometry(1, cScale / 40, cScale / 40);
      let cElem = new Mesh(cGeo, cMat);
      let cAmp = 3;
